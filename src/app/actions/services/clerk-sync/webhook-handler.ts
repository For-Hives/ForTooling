...
import * as organizationService from "../pocketbase/organizationService";
import * as userService from "../pocketbase/userService";
 ...
 */
export async function processWebhookEvent(event: WebhookEvent): Promise<WebhookProcessingResult> {
  ...
  try {
    ...
    if (event.type === "organization.created") {
      return await organizationService.handleWebhookCreated(event.data, elevated);
    } 
    else if (event.type === "organization.updated") {
      return await organizationService.handleWebhookUpdated(event.data, elevated);
    } 
    else if (event.type === "organization.deleted") {
      return await organizationService.handleWebhookDeleted(event.data, elevated);
    }
    
    // Handle membership events
    else if (event.type === "organizationMembership.created") {
      return await organizationService.handleMembershipWebhookCreated(event.data, elevated);
    } 
    else if (event.type === "organizationMembership.updated") {
      return await organizationService.handleMembershipWebhookUpdated(event.data, elevated);
    } 
    else if (event.type === "organizationMembership.deleted") {
      return await organizationService.handleMembershipWebhookDeleted(event.data, elevated);
    }
    ...
    // Unknown event type
      ...
      return { success: false, message: `Unhandled webhook event type: ${event.type}` };
    }
  } catch (error) {
